...
import ramjidentalcom.ramjidentalservices.AdminPanel.Registrations.ClientRegistration;
import ramjidentalcom.ramjidentalservices.HomePage;
import ramjidentalcom.ramjidentalservices.R;

public class ClientScanner extends AppCompatActivity implements ZXingScannerView.ResultHandler {
...
protected void onCreate(Bundle savedInstanceState) {
        ...
        al = new AlertDialog.Builder(ClientScanner.this).create();
        ...
        public void onBackPressed() {
                Intent intent = new Intent(ClientScanner.this, AdminDashboard.class);
        ...
        class FetchInBackground extends AsyncTask<String, Void, String> {
                ...
                protected String doInBackground(String... params) {
                        String json_url = "http://ramjidental.com/RamjiApp/checkclntid.php";
                        String result = null;
                        try {
        ...
        private void got(){
                Intent intent = new Intent(ClientScanner.this,ClientRegistration.class);
        ...
        public boolean onOptionsItemSelected(MenuItem item) {
                switch (item.getItemId()){
                        case R.id.logout:
                                Intent intent;
                                intent = new Intent(ClientScanner.this, HomePage.class);
        ...
        public void handleResult(Result result) {
                ex = result.getText();
                if((ex.matches("(.*)clnt(.*)"))&&(!ex.matches("(.*)clntcust(.*)"))) {
                        if (ex.contains(":")) {
                                String[] s = ex.split(":");
                                id = s[0];
                                String clnt = s[1];
                        ...
                        al.setMessage("This is not a Client Card");
                        al.setButton(DialogInterface.BUTTON_NEUTRAL, "CLOSE", new DialogInterface.OnClickListener() {
                                @Override
                                public void onClick(DialogInterface dialog, int which) {
                                        dialog.dismiss();
